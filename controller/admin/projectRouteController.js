...
const addProjectRoute = async (req, res) => {
  try {
    ...
    const validateRequest = validation.validateParamsWithJoi(dataToCreate, projectRouteSchemaKey.schemaKeys);
    if (!validateRequest.isValid) {
      return res.validationError({ message: `Invalid values in parameters, ${validateRequest.message}` });
    }
    dataToCreate.addedBy = req.user.id;
    dataToCreate = new ProjectRoute(dataToCreate);
    const result = await dbService.createDocument(ProjectRoute, dataToCreate);
...
const bulkInsertProjectRoute = async (req, res) => {
  try {
    ...
    const result = await dbService.bulkInsert(ProjectRoute, dataToCreate);
    return res.success({ data: result });
  } catch (error) {
    if (error.name === 'ValidationError') {
      return res.validationError({ message: error.message });
...
const findAllProjectRoute = async (req, res) => {
  try {
    ...
    if (req.body.isCountOnly) {
      const totalRecords = await dbService.countDocument(ProjectRoute, query);
      return res.success({ data: { totalRecords } });
    }

    const result = await dbService.getAllDocuments(ProjectRoute, query, options);
...
const getProjectRoute = async (req, res) => {
  try {
    ...
    const result = await dbService.getDocumentByQuery(ProjectRoute, query);
    if (!result) {
      return res.recordNotFound();
    }
    return res.success({ data: result });
...
const partialUpdateProjectRoute = async (req, res) => {
  try {
    ...
    const query = { _id: req.params.id };
    const result = await dbService.findOneAndUpdateDocument(ProjectRoute, query, dataToUpdate);
    if (!result) {
      return res.recordNotFound();
    }
    return res.success({ data: result });
...
const softDeleteProjectRoute = async (req, res) => {
  try {
    ...
    const query = { _id: req.params.id };
    const result = await deleteDependentService.softDeleteProjectRoute(query, req.user);
    if (!result) {
      return res.recordNotFound();
    }
    return res.success({ data: result });
...
const getProjectRouteCount = async (req, res) => {
  try {
    ...
    const totalRecords = await dbService.countDocument(ProjectRoute, where);
    return res.success({ data: { totalRecords } });
  } catch (error) {
    return res.internalServerError({ message: error.message });
  }
...
const bulkUpdateProjectRoute = async (req, res) => {
  try {
    ...
    dataToUpdate.updatedBy = req.user.id;
    const result = await dbService.bulkUpdate(ProjectRoute, filter, dataToUpdate);
    if (!result) {
      return res.recordNotFound();
    }
    return res.success({ data: result });
