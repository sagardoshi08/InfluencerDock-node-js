...
const addUserRole = async (req, res) => {
  try {
    ...
    const validateRequest = validation.validateParamsWithJoi(dataToCreate, userRoleSchemaKey.schemaKeys);
    if (!validateRequest.isValid) {
      return res.validationError({ message: `Invalid values in parameters, ${validateRequest.message}` });
    }
    dataToCreate.addedBy = req.user.id;
    dataToCreate = new UserRole(dataToCreate);
    const result = await dbService.createDocument(UserRole, dataToCreate);
...
const bulkInsertUserRole = async (req, res) => {
  try {
    ...
    const result = await dbService.bulkInsert(UserRole, dataToCreate);
    return res.success({ data: result });
  } catch (error) {
    if (error.name === 'ValidationError') {
      return res.validationError({ message: error.message });
...
const findAllUserRole = async (req, res) => {
  try {
    ...
    if (req.body.isCountOnly) {
      const totalRecords = await dbService.countDocument(UserRole, query);
      return res.success({ data: { totalRecords } });
    }

    const result = await dbService.getAllDocuments(UserRole, query, options);
...
const getUserRole = async (req, res) => {
  try {
    ...
    const result = await dbService.getDocumentByQuery(UserRole, query);
    if (!result) {
      return res.recordNotFound();
    }
    return res.success({ data: result });
...
const partialUpdateUserRole = async (req, res) => {
  try {
    ...
    const query = { _id: req.params.id };
    const result = await dbService.findOneAndUpdateDocument(UserRole, query, dataToUpdate);
    if (!result) {
      return res.recordNotFound();
    }
    return res.success({ data: result });
...
const softDeleteUserRole = async (req, res) => {
  try {
    ...
    const query = { _id: req.params.id };
    const result = await dbService.findOneAndUpdateDocument(UserRole, query, { isDeleted: true });
    if (!result) {
      return res.recordNotFound();
    }
    return res.success({ data: result });
...
const getUserRoleCount = async (req, res) => {
  try {
    ...
    const totalRecords = await dbService.countDocument(UserRole, where);
    return res.success({ data: { totalRecords } });
  } catch (error) {
    return res.internalServerError({ message: error.message });
  }
...
const bulkUpdateUserRole = async (req, res) => {
  try {
    ...
    dataToUpdate.updatedBy = req.user.id;
    const result = await dbService.bulkUpdate(UserRole, filter, dataToUpdate);
    if (!result) {
      return res.recordNotFound();
    }
    return res.success({ data: result });
